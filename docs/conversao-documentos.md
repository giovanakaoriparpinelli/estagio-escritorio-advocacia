...
Foi criada a ferramenta `scripts/md_to_docx.py` para converter os Markdown gerados pelo protótipo em DOCX baseado no arquivo `identidade-visual/modelo timbrado.docx`.
...
```powershell
python scripts/md_to_docx.py rascunho.md --template "identidade-visual\modelo timbrado.docx" --output documentos\manifestacao.docx
```
